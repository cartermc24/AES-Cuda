...
int main(int argc, const char * argv[])
{
	...
	cudaDevAssist(cudaMemcpyToSymbol(Rcon, &Rcon_h, 256*sizeof(uint8_t), 0, cudaMemcpyHostToDevice), 534, true);
	...
	cudaThreadSynchronize();
	cudaDevAssist(cudaMemcpyToSymbol(ek, &eK, 44*sizeof(uint32_t), 0, cudaMemcpyHostToDevice), 823, true);
	...
	while (end)
	{
		...
		for (int i = 0; i < spawn; i++)
		{
			...
			for (int c = 0; c < 16; c++) { memcpy(&states[0][c], &temp[c], sizeof(uint8_t)); }
		}
